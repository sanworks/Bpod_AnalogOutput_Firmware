...
const byte nChannels = 8;
const byte nCustomPulseTrains = 2;
...
// Parameters that define pulse trains currently loaded on the output channels
...
unsigned long CustomTrainNpulses[nCustomPulseTrains] = {0}; // Stores the total number of pulses in the custom pulse train
...
unsigned long CustomPulseTimes[nCustomPulseTrains][10001] = {0};
uint16_t CustomVoltages[nCustomPulseTrains][10001] = {0};
    ...
    if (CommandByte == OpMenuByte) { // The first byte must be 213. Now, read the actual command byte. (Reduces interference from port scanning applications)
      ...
      switch (CommandByte) {
        ...
        case 74: { // Program one parameter, one channel
          ...
          inByte3 = PPUSB.readByte(); // inByte3 = target channels (1-nChannels)
       ...
       case 81: { // Disconnect from client
        ...
        case 91: { // Program a parameter on all channels
          inByte2 = PPUSB.readByte();
          switch (inByte2) { 
